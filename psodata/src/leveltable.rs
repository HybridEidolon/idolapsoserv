...
impl Serial for StartStats {
    ...
    fn deserialize(src: &mut Read) -> io::Result<Self> {
        ...
        let lck = try!(Serial::deserialize(src));
        Ok(StartStats {
            atp: atp,
            mst: mst,
            evp: evp,
...
pub struct LevelTable {
    pub start_stats: Vec<StartStats>,
    unk: Vec<u32>,
...
impl Serial for LevelTable {
    fn serialize(&self, dst: &mut Write) -> io::Result<()> {
        ...
        try!(write_array(&self.unk, 12, dst));
